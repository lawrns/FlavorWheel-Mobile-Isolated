...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_reviews' AND column_name = 'typicicty_score') THEN
        ALTER TABLE user_reviews ADD COLUMN typicity_score integer CHECK (typicicty_score >= 0 AND typicity_score <= 100);
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_user_reviews_typicity_score ON user_reviews(typicicty_score);
...
COMMENT ON COLUMN user_reviews.typicicty_score IS 'How typical/representative of style (0-100)';
